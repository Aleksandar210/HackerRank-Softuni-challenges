...
namespace SoftuniFundamentals
{
    class ArraysMethodsLists
    {
        ...
        public void ThereIsNoSpoonTask()
        {

            Console.Write("Enter height: ");
            int heightOfGrid = int.Parse(Console.ReadLine());
            Console.Write("Enter Width: ");
            int widthOfGrid = int.Parse(Console.ReadLine());
            char[][] currentGrid = new char[heightOfGrid][];
            Console.Clear();

            for (int i = 0; i < heightOfGrid; i++)
            {
                string rowLine;
                Console.Write("Enter line content(0 /. ): ");
                rowLine = Console.ReadLine();
                if (rowLine.Split(" ", StringSplitOptions.RemoveEmptyEntries).Length != widthOfGrid)
                {
                    i--;
                }
                else
                {
                    this.EnterLineContentInGridSpoonTask(currentGrid, i, rowLine);
                }
            }


            List<string> currentNodeResults = new List<String>();
            this.GetResultLinesWithEnergyNodesAndTheirNeighbours(currentGrid, currentNodeResults, this.GetLocationsOfNodesWithEnergy(currentGrid));
            if(currentNodeResults.Count==0)
            {
                Console.WriteLine("No Energy Nodes in the grid");
            }
            else
            {
                foreach (var item in currentNodeResults)
                {
        ...
        private void EnterLineContentInGridSpoonTask(char[][] currentGrid, int index, string lineContents)
        {
            char[] currentLineArray = lineContents.Split(" ", StringSplitOptions.RemoveEmptyEntries).Select(e => char.Parse(e)).ToArray();
            currentGrid[index] = currentLineArray;
        }

        private string[] GetLocationsOfNodesWithEnergy(char[][] currentGrid)
        {
            StringBuilder locationsOfNodesWithEnergy = new StringBuilder();
            for (int i = 0; i < currentGrid.Length; i++)
            {
                for (int j = 0; j < currentGrid[i].Length; j++)
                {
                    if (currentGrid[i][j].Equals('0'))
                    {
                        locationsOfNodesWithEnergy.Append(i + " " + j + ",");
                    }
                }
            }

            return locationsOfNodesWithEnergy.ToString().Trim(new char[] { ',' }).Split(",");
        }

        private void GetResultLinesWithEnergyNodesAndTheirNeighbours(char[][] currentGrid,List<string> currentResultLines ,string[] locationsOfEnergyLines)
        {
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < locationsOfEnergyLines.Length; i++)
            {
                int nodeRowLocation = int.Parse(locationsOfEnergyLines[i].Split(" ", StringSplitOptions.RemoveEmptyEntries)[0]);
                int nodeColLocation = int.Parse(locationsOfEnergyLines[i].Split(" ", StringSplitOptions.RemoveEmptyEntries)[1]);
                sb.Append(nodeRowLocation + " " + nodeColLocation + ",");
                if(this.HasRightNeighbour(nodeRowLocation,nodeColLocation,currentGrid))
                {
                    sb.Append(nodeRowLocation + " "+(nodeColLocation + 1) + ",");
                }
                else
                {
                    sb.Append("-1 -1,");
                ...
                if(this.HasDownNeighbourNeighbour(nodeRowLocation,nodeColLocation,currentGrid))
                {
                    sb.Append((nodeRowLocation+1) + " " + (nodeColLocation) + ",");
                }
                else
                {
                    sb.Append("-1 -1,");
                ...
                currentResultLines.Add(sb.ToString().Trim(new char[] { ',' }));
                sb.Clear();
            }
        ...
        private bool HasRightNeighbour(int rowLocation,int colLocation,char[][] currentGrid)
        {
            int gridLastColIndex = currentGrid[rowLocation].Length - 1;
            if(colLocation+1<=gridLastColIndex)
            {
                return true;
            }
            else
        ...
        private bool HasDownNeighbourNeighbour(int rowLocation, int colLocation, char[][] currentGrid)
        {
            int gridRowLastindex = currentGrid.Length - 1;
            int gridLastColIndex = currentGrid[rowLocation].Length - 1;
            if (rowLocation+1<=gridRowLastindex)
            {
                return true;
            }
            else
